...
public class UpiIndiaPlugin
    ...
    private void startTransaction(MethodCall call) {
        ...
        try {
            ...
            String uriString = "upi://pay?appid=inb_admin&tr=IND1e93cfea70e34f2aad3192c5bfd7a5f2&am="
                    + Uri.encode(amount) +
                    "&mc=&pa=9992211889@indianbk&pn=VAULT SECURITIES SERVICES&tn="
                    + (transactionNote != null ? Uri.encode(transactionNote) : "") + "&am=" + Uri.encode(amount)
                    + "&cu=&bn=VAULT SECURITIES SERVICES";
            // String uriString = "upi://pay?pa="+receiverUpiId+
            // "&pn="+Uri.encode(receiverName)+
            // "&am="+Uri.encode(amount);
            // if (transactionNote != null) uriString += "&tn=" +
            // Uri.encode(transactionNote);
            // if (transactionRefId != null) uriString += "&tr=" +
            // Uri.encode(transactionRefId);
